...
def checkLeftRight(image, results, draw=False, display=False):
    ...
    left_x = int(results.pose_landmarks.landmark[mp_pose.PoseLandmark.RIGHT_PINKY].x * width)

    # Retreive the x-corrdinate of the right shoulder landmark.
    right_x = int(results.pose_landmarks.landmark[mp_pose.PoseLandmark.LEFT_PINKY].x * width)

    # print("left hand x: ", left_x, "right hand x: ", right_x)
    left_shoulder_x = int(results.pose_landmarks.landmark[mp_pose.PoseLandmark.RIGHT_SHOULDER].x * width)
    right_shoulder_x = int(results.pose_landmarks.landmark[mp_pose.PoseLandmark.LEFT_SHOULDER].x * width)

    # Check if the person is at left that is when left or right PINKY landmarks x-corrdinates
    # are <= the x-corrdinate of the left shoulder.
    if (right_x <= left_shoulder_x or left_x <= left_shoulder_x):

        # Set the person's position to left.
        horizontal_position = 'Left'

    # Check if the person is at left that is when left or right PINKY landmarks x-corrdinates
    # are >= the x-corrdinate of the right shoulder.
    elif (right_x >= right_shoulder_x or left_x >= right_shoulder_x):
...
def checkJumpCrouch(image, results, MID_Y=300, draw=False, display=False):
    '''
    ...
    shoulder_mid_y = abs(right_y + left_y) // 2

    left_elbow_y = int(results.pose_landmarks.landmark[mp_pose.PoseLandmark.RIGHT_ELBOW].y * height)
    right_elbow_y = int(results.pose_landmarks.landmark[mp_pose.PoseLandmark.LEFT_ELBOW].y * height)
    elbow_mid_y = (left_elbow_y + right_elbow_y) / 2

    left_y = int(results.pose_landmarks.landmark[mp_pose.PoseLandmark.RIGHT_PINKY].y * height)

    # Retreive the x-corrdinate of the right shoulder landmark.
    right_y = int(results.pose_landmarks.landmark[mp_pose.PoseLandmark.LEFT_PINKY].y * height)

    pinky_mid_y = (left_y + right_y) / 2

    # Calculate the upper and lower bounds of the threshold.
    lower_bound = elbow_mid_y
    upper_bound = shoulder_mid_y
...
"""Now we will test the function **`checkJumpCrouch()`** created above on the real-time webcam feed and will visualize the resultant frames. For testing purposes, we will be using a default value of the threshold, that if you want you can tune manually set according to your hands movement."""
...
pyautogui.click(x=800, y=565, button='right')
...
while camera_video.isOpened():
    ...
    if results.pose_landmarks:
        ...
        if game_started:
            ...
            print(horizontal_position, x_pos_index)
        ...
        if checkHandsJoined(frame, results)[1] == 'Hands Joined' and not game_started:
            ...
            if counter == num_of_frames:
                ...
                if not (game_started):
                    ...
                    # Move to 1300, 800, then click the left mouse button to start the game.
                    pyautogui.click(x=800, y=565, button='left')
                    #This for maximize the video game vindow
                    pyautogui.click(x=1320, y=850, button='left')
    ...
    # Check if the dif  ference between the previous and this frame time > 0 to avoid division by zero.
    if (time2 - time1) > 0:
...
"""## This is our final application using the `Mediapipe`,`open-cv`,`pyautogui` library."""
